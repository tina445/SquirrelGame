...
구현 순서는 명세와 동일하게 `P0 기반/공유 규칙 → P1 회색상자 네트워크 이동 → P2 절차적 맵 → P3 8인 Room/도토리 → P4 체포/감옥/구출 → P5 베리/람쥐썬더 → P6 안정화`입니다. P0~P5 자동 검증과 포스트 MVP 로비/매칭 1차 수직 절편은 완료되었습니다. 다음 우선순위는 실제 8인 로비→매치 플레이테스트, 방 코드 복사·카운트다운 표시 같은 UX 폴리싱, 연결 그래프·우회로·병목 점수까지 포함하는 맵 topology 확장, 연출 보강, WSS 배포 리허설입니다.
