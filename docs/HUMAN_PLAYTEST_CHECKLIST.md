...
- Leave berries uncollected for at least 70 seconds. Confirm up to five appear, each is visibly separated across the map rather than clustered around one objective, and no berry overlaps a blocker or spawn area.
...
- Approach the thief base, each police storage, jail, a usable acorn, and an arrestable thief. Confirm the larger translucent tooltip stays above rather than covering its player/prefab and disappears out of range. With two or more prisoners, the single `[Space] 동료 구출` tooltip must remain anchored to the jail prefab rather than one prisoner.
