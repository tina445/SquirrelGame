import { clampMagnitude, gameBalance, moveCircle, movementCircleColliders, scale, subtract, type CircleCollider, type InputCommand, type MapDefinition, type PlayerSnapshot, type Vec2 } from '@squirrel-heist/shared';
...
export class LocalPrediction {
  ...
  apply(input: InputCommand, deltaSeconds: number, carrying: boolean): void {
    ...
    this.position = this.move(this.position, { x: input.moveX, y: input.moveY }, deltaSeconds, carrying);
  }
  ...
  reconcile(authoritative: PlayerSnapshot): void {
    ...
    for (const input of this.pending) replay = this.move(replay, { x: input.moveX, y: input.moveY }, 1 / gameBalance.serverTickRate, authoritative.heldAcornId !== null);
  ...
  advanceVisual(localMovement: Vec2, deltaSeconds: number, carrying: boolean, movable: boolean): Vec2 {
    if (!this.map) return this.visualPosition;
    const safeDelta = Math.max(0, Math.min(0.05, deltaSeconds));
    if (movable) this.visualPosition = this.move(this.visualPosition, localMovement, safeDelta, carrying);
  ...
  /** 서버와 같은 월드 축 이동속도·운반 감속·벽·hole·나무 줄기·감옥 경계 충돌을 적용한다. */
  private move(position: Vec2, movement: Vec2, deltaSeconds: number, carrying: boolean): Vec2 {
    const direction = clampMagnitude(movement);
